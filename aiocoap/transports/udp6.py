...
class TransportEndpointUDP6(RecvmsgDatagramProtocol, interfaces.TransportEndpoint):
    def __init__(self, new_message_callback, new_error_callback, log, loop, secure=False):
        ...
        # FIXME DTLS should be optional
        self._is_dtls=secure

        self.security = DTLSSecurityStore()
        self._connecting = False

        # FIXME The port sould be provided by the user. For now, this "sentinel" is enough.
        pskId, psk = self.security._get_psk("::1", COAPS_PORT)

        # dtls.setLogLevel(dtls.DTLS_LOG_DEBUG)
        dtls.setLogLevel(dtls.DTLS_LOG_INFO)
        self.log.debug("TinyDTLS Log level ",dtls.dtlsGetLogLevel() )
    ...
    def _dtls_event(self, level, code):
        if (level, code) == (LEVEL_NOALERT, DTLS_EVENT_CONNECT):
            self.log.debug("DTLS-DEBUG: Client Connect")
            self._connecting = False
            return
        elif (level, code) == (LEVEL_NOALERT, DTLS_EVENT_CONNECTED):
            self.log.debug("DTLS-DEBUG: Client handshake finished!")
    ...
    @classmethod
    @asyncio.coroutine
    def _create_transport_endpoint(cls, new_message_callback, new_error_callback, log, loop, dump_to, bind, multicast=False, secure=False):
        protofact = lambda: cls(new_message_callback=new_message_callback, new_error_callback=new_error_callback, log=log, loop=loop, secure=secure)
    ...
    @classmethod
    @asyncio.coroutine
    def create_server_transport_endpoint(cls, new_message_callback, new_error_callback, log, loop, dump_to, bind, secure):
        return (yield from cls._create_transport_endpoint(new_message_callback, new_error_callback, log, loop, dump_to, bind, multicast=True, secure=secure))
    ...
    def send(self, message):
        ...
        if self._is_dtls:
            # FIXME This should be with a transport too
    ...
    def _dtls_msg_received(self, data, ancdata, flags, address):
        pktinfo = None
    ...
    def _udp_msg_received(self, data, ancdata, flags, address):
    ...
    def datagram_msg_received(self, data, ancdata, flags, address):
        ...
        if self._is_dtls:
            self._dtls_msg_received( data, ancdata, flags, address)
        else:
            self._udp_msg_received( data, ancdata, flags, address)
